...
if (download_and_process) {
  ...
  df_oecd_phs <- readr::read_csv("Project_dh2025/data/oecd_phs.csv")
...
ggplot(df_plot, aes(
  ...
  geom_point(
    data = df_plot |>
      filter(country_name %in% c("United States", "Switzerland")),
    aes(x = ind_WorldBank_health_pp,
        y = ind_WorldBank_lifeexpectancy),
    color = "#D55E00",
    size = 3.3
  ) +
...
ggplot(df_plot, aes(
  ...
  geom_text_repel(
    data = df_plot |>
      filter(!is.na(label) &
               !(country_name %in% c("United States", "Switzerland"))),
    aes(label = label),
    size = 3,
    color = "black",
    segment.color = "grey80"
  ...
  labs(
    title = "Twice the spending, same or lower life expectancy",
    subtitle = "US and Switzerland spend about twice as much on health (PPP per capita, 2022) as other OECD countries",
    x = "Health expenditure per capita (PPP)",
    y = "Life Expectancy at Birth (years)"
  ) +
